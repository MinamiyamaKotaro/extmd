...
| 必須 | `y`/`m`/`d`/`h`/`s` 等の日付トークンを含むカスタム書式 | 簡易的なトークン検出（`src/reader/date.rs`の`contains_date_token`として実装済み） |
| 対象外 | 日付トークンを含まない書式なのに実質日付として使われているセル（誤検出源） | 検出不能として`Number`扱いにする |

当初はこの判定ロジックが`ssfmt`（採用候補、[cell.md 4章](../domain/cell.md#4-format_number--format_date-のフォーマット方針)）の
書式パーサと重複する可能性を懸念し、採用が決まった場合の流用を実装フェーズで検討する
としていたが、[Issue #2でのスパイク検証](https://github.com/MinamiyamaKotaro/extmd/issues/2#issuecomment-5304633010)の結果
`ssfmt`はv1で不採用と決定したため、この重複整理は不要になった。本ファイルの
トークン検出ロジックが最終実装である。
