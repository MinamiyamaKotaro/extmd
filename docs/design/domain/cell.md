...
| v1対象外 | 和暦、通貨記号、数値書式へのテキスト埋め込み（例: `0"円"`）等のロケール依存書式 | 下記の理由によりv1では自前実装の対象から外す |
| 対象外 | 文字色指定、条件付き表示、文字埋め等の視覚的装飾 | Markdownのプレーンテキストで再現できないためストリップ（無視）する |

**「v1対象外」の「テキスト埋め込み」は数値書式（`format_number`）への任意の装飾テキスト
付与を指し、日付書式（`format_date`/`render_date_tokens`）が`"..."`で区切り文字を囲む
構文（例: `yyyy"年"m"月"`）とは別の話である。** 後者はExcelの書式コード構文上、
引用符自体を出力に含めず中身だけを挿入する意味を持つ、日本の業務Excelで極めて頻出する
基本パターンであり、「基本的な日付/時刻」（必須スコープ）に含めて対応する。実データで
引用符自体（`"`）が出力に混入する不具合（例: `2019"年"1"月"`）が判明したため、
`render_date_tokens`で`"..."`（引用符を除いて中身のみ出力）・`\X`（バックスラッシュを
除いてXのみ出力）を処理するよう修正した。
