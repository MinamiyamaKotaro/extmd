...
| 優先度 | 対象 | 方針 |
|---|---|---|
| 必須（v1スコープ） | 桁区切り、小数点以下桁数、パーセンテージ、基本的な日付/時刻（`yyyy/mm/dd hh:mm`）、`"..."`/`\X`によるリテラルテキストの埋め込み（例: `yyyy"年"m"月"`、`"("#"ヶ月間)"`） | これらが欠けると生データ（シリアル値等）がそのまま出力され可読性が著しく低下する、または実データで頻出するため必須 |
| v1対象外 | 和暦、`[$currency-lcid]`形式の通貨記号等のロケール依存書式 | 下記の理由によりv1では自前実装の対象から外す |
| 対象外 | 文字色指定、条件付き表示、文字埋め等の視覚的装飾 | Markdownのプレーンテキストで再現できないためストリップ（無視）する |

**`"..."`/`\X`によるリテラルテキストの埋め込みは、日付書式（`format_date`/
`render_date_tokens`）・数値書式（`format_number`/`render_number_literals`）の
両方で対応している。** Excelの書式コード構文上、`"..."`は引用符自体を出力に含めず
中身だけを挿入する意味を持ち、`yyyy"年"m"月"`（日付の区切り文字）・`"("#"ヶ月間)"`
（数値の前後への単位・括弧の付与）のいずれも日本の業務Excelで極めて頻出する基本
パターンである。日付側は実データで引用符自体（`"`）が出力に混入する不具合
（例: `2019"年"1"月"`、Issue報告）が判明し先に修正（PR #44）、その後数値側でも
同種の構文（`"("#"ヶ月間)"`が`12`としか出力されず単位・括弧が欠落する不具合）が
実データで確認されたため（[Issue #45](https://github.com/MinamiyamaKotaro/extmd/issues/45)）、
`render_number_literals`として同じ構文解釈を追加した。
「v1対象外」の通貨記号は、これとは異なる`[$...]`ブラケット構文であり別対応が必要。
